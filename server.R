...
shinyServer(function(input, output, session) {
  zipdata<- reactive({
    ...
    if(input$species=='punctigera'){
      return(subset(cleantable,
                    as.Date(date) >= input$dateMin&
                      as.Date(date) <= as.Date(input$dateMin+input$timeSpan*7, origin = '1970-01-01') &
  ...
  output$yearSlider <- renderUI({
    input$refresh
    sliderInput('date', 'Map showing trap data for date range:', min=input$dateMin, max=input$dateMin+input$timeSpan*7, value = c(input$dateMin, input$dateMin+input$binSize*7), width = '100%', step = input$binSize*7,animate = animationOptions(interval = input$aniSpeed*1000/as.numeric((input$timeSpan)/(input$binSize)),loop=TRUE))
   
  })
  output$durationWarning<-renderText({
    if(input$timeSpan%%input$binSize!=0){
  ...
  legendMax<-reactive({
    # browser()
    df <- subset(zipdata(), date >=input$dateMin&
                   date<=as.Date(input$dateMin+input$timeSpan*7, origin = '1970-01-01' ))
    df$variablebin<-as.Date(NA, origin = "1970-1-1")
    datebins<-seq(input$dateMin,as.Date(input$dateMin+input$timeSpan*7, origin = '1970-01-01'), by = 7*ifelse(is.na(input$binSize),1,input$binSize))
